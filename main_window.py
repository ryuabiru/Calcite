...
class MainWindow(QMainWindow):
    ...
    def save_graph(self):
        ...
        if file_path:
            try:
                ...
                QMessageBox.information(self, "Success", f"Graph successfully saved to:{file_path}")
    ...
    def update_graph_properties(self):
        ...
        # スケール設定ロジックを修正
        # 棒グラフやペアプロットの場合、X軸は必ず'linear'（カテゴリ表示）にする
        if self.current_graph_type in ['bar', 'paired_scatter']:
